...
from agent.world_models.diffusion.denoiserjax import Denoiser as DenoiserJax
from agent.world_models.diffusion.diffusion_samplerjax import DiffusionSampler as DiffusionSamplerJax, DiffusionSamplerConfig, sample_jit
# from agent.world_models.diffusion import Denoiser, DiffusionSampler, DiffusionSamplerConfig
from agent.world_models.rew_end_modeljax_eqx import TransRewEndModel, RewEndModel,get_act_emb
...
class WorldModelEnv:
    def __init__(
        ...
        **kwargs,
        ...
        self.sampler = DiffusionSamplerJax(denoiser, cfg.diffusion_sampler)
        self.ByJax = True
        ...
        self.sl = denoiser.steps_condition
    ...
    def predict_next_state(self) -> Tuple[Tensor, List[Tensor]]:
        ...
        if self.ByJax:
            state_buffer_jax = jnp.array(self.state_buffer.cpu().numpy())
            act_buffer_jax = jnp.array(self.act_buffer.cpu().numpy())
            if self.mode == 'ensemble':
                result = self.sampler.ensemble_sample(state_buffer_jax, act_buffer_jax, key=subkey)
            else:
                result = sample_jit(self.sampler,state_buffer_jax, act_buffer_jax, key=subkey)
                # result = self.sampler.sample(state_buffer_jax, act_buffer_jax, key=subkey)
            result = torch.from_numpy(np.array(result)).to(self.device)
            # trajectory = [torch.from_numpy(np.array(t)).to(self.device) for t in trajectory]
            return result, None
        else:
            if self.mode == 'ensemble':
                return self.sampler.ensemble_sample(self.state_buffer, self.act_buffer, key=subkey)
            else:
                return self.sampler.sample(self.state_buffer, self.act_buffer, key=subkey)
    ...
    def predict_rew_end(self, next_state: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if self.ByJax:
            state_buffer_jax = jnp.array(self.state_buffer.cpu().numpy())
            # act_buffer_jax = jnp.array(self.act_buffer.cpu().numpy())
            current_state = self.sampler.decode(state_buffer_jax[:, -1:])
            # act_cond = self.rew_end_model.get_act_emb(act_buffer_jax[:, -1])
            # act_cond = torch.from_numpy(np.array(act_cond)).to(self.device).unsqueeze(1)
            act_buffer_jax = jnp.array(self.act_buffer[:, -1].cpu().numpy())
            # act_cond = self.rew_end_model.get_act_emb(act_buffer_jax)
            act_cond = get_act_emb(self.rew_end_model.act_emb, self.rew_end_model.is_discrete_action,act_buffer_jax)
            act_cond = torch.from_numpy(np.array(act_cond)).to(self.device)
            
            current_state= torch.from_numpy(np.array(current_state)).to(self.device)
            # act_cond = torch.from_numpy(np.array(act_cond)).to(self.device)
        else:
        ...
        outputs_rew_end = self.rew_end_model(input_rew_end, perattn_out=act_cond.unsqueeze(1), past_keys_values = self.keys_values_rew_end, attention_mask = torch.flip(self.flipped_attn_mask, dims=[-1]))
