...
class InnerModel(nn.Module):
    ...
    def forward(self, noisy_next_obs: Tensor, c_noise: Tensor, obs: Tensor, act: Tensor, act_mask: Tensor) -> Tensor:
        ...
        # start_time = time.time()
        x = self.conv_in(x)
        # self.time_c[3] += time.time() - start_time
        # start_time = time.time()
        x, _, _ = self.unet(x, cond)#!
        # self.time_c[4] += time.time() - start_time
        # start_time = time.time()
        x = self.conv_out(F.silu(self.norm_out(x)))
        # self.time_c[5] += time.time() - start_time
        # start_time = time.time()
        
        x = rearrange(x, 'b t h -> b h t')
        # self.time_c[6] += time.time() - start_time     
