...
# world model related - PyTorch versions
from agent.world_models.diffusion import Denoiser as DenoiserTorch, DiffusionSampler as DiffusionSamplerTorch
# JAX versions
from agent.world_models.diffusion.denoiserjax import Denoiser as DenoiserJax, DenoiserConfig, SigmaDistributionConfig
from agent.world_models.diffusion.diffusion_samplerjax import DiffusionSampler as DiffusionSamplerJax, DiffusionSamplerConfig
...
from utils import configure_optimizer,configure_optimizer_jax
...
# Define JAX training step
@eqx.filter_jit
def train_step(denoiser, denoiser_opt,opt_state, shared_obs, act, mask_padding, key):
    def loss_fn(model):
        loss, metrics = model(shared_obs, act, mask_padding, key)
        return loss, metrics
    
    (loss, metrics), grads = eqx.filter_value_and_grad(loss_fn, has_aux=True)(denoiser)
    updates, new_opt_state = denoiser_opt.update( grads, opt_state, eqx.filter(denoiser, eqx.is_array) )
    new_denoiser = eqx.apply_updates(denoiser, updates)
    return new_denoiser, new_opt_state, loss, metrics
@eqx.filter_jit
def train_step_TransRewEndModel(rew_end_model, denoiser_opt,opt_state, samples, attn_mask, key):
    def loss_fn(model):
        loss, metrics = rew_end_model.compute_loss(samples, attn_mask)
        return loss, metrics
    
    (loss, metrics), grads = eqx.filter_value_and_grad(loss_fn, has_aux=True)(rew_end_model)
    updates, new_opt_state = denoiser_opt.update( grads, opt_state, eqx.filter(rew_end_model, eqx.is_array) )
    new_model = eqx.apply_updates(rew_end_model, updates)
    return new_model, new_opt_state, loss, metrics

@eqx.filter_jit
def train_step_RNNRewEndModel(rew_end_model, denoiser_opt,opt_state, samples, gamma, contdisc, key):
    def loss_fn(model):
        loss, metrics = rew_end_model(samples, gamma = gamma, contdisc = contdisc)
        return loss, metrics
...
class DreamerLearner:
    ...
    def __init__(self, config):
        ...
        self.denoiser = self.denoiser.setup_training(self.config.sigma_distribution)
        
        # Store device for torch compatibility
        ...
        if config.rew_end_model_type == "rnn":
            self.rew_end_model = RewEndModelJax(
            ...
            # import pickle
            # pickle.dump([self.config,(self.env_type in [Env.STARCRAFT, Env.SMACv2])], open('rewendmodel_config.pkl', 'wb'))
            # exit()
            
            self.rew_end_model = TransRewEndModelJax(
                state_dim=config.STATE_DIM,
        ...
        print("")
        # Count JAX denoiser parameters
        denoiser_params = sum(x.size for x in jax.tree_util.tree_leaves(eqx.filter(self.denoiser, eqx.is_array)))
        print(f"{denoiser_params} parameters in denoiser (JAX)")
        print(f"{count_parameters(self.state_decoder)} parameters in state_decoder")
        # print(f"{count_parameters(self.rew_end_model)} parameters in rew_end_model")
        rew_params = sum(x.size for x in jax.tree_util.tree_leaves(eqx.filter(self.rew_end_model, eqx.is_array)))
        print(f"{rew_params} parameters in rew_end_model (JAX)")
    ...
    def init_optimizers(self):
        ...
        # JAX/Optax optimizer for denoiser
        lr = self.config.denoiser_opt_cfg.get('lr', 3e-4)
        warmup_steps = self.config.denoiser_lr_warmup_steps
        # Create optax optimizer with warmup schedule
        ...
        self.denoiser_opt = optax.chain(
            optax.clip_by_global_norm(self.config.denoiser_max_grad_norm or 1.0),
            optax.adamw(learning_rate=schedule),
        )
        
        self.denoiser_opt_state = self.denoiser_opt.init(eqx.filter(self.denoiser, eqx.is_array))
        # # JAX/Optax optimizer for rew_end_model
        self.rew_end_model_opt=optax.chain(
            optax.clip_by_global_norm(self.config.remodel_max_grad_norm or 1.0),
            optax.adamw(learning_rate=self.config.rew_end_model_opt_cfg['lr'],
                        weight_decay=self.config.rew_end_model_opt_cfg['weight_decay'])
        )
        self.rewend_optstate = self.rew_end_model_opt.init(eqx.filter(self.rew_end_model, eqx.is_array))
        ...
        # PyTorch optimizers for actor and critic
    ...
    def params(self):
        return {'state_decoder': {k: v.cpu() for k, v in self.state_decoder.state_dict().items()},
                ...
                # 'denoiser': eqx.tree_serialise_leaves(self.denoiser),
    ...
    def load_pretrained(self, load_path):
        ...
        if 'denoiser' in ckpt:
            # Load JAX denoiser from serialized leaves
            self.denoiser = eqx.tree_deserialise_leaves(ckpt['denoiser'], self.denoiser)

        if 'rew_end_model' in ckpt:
            self.rew_end_model=eqx.tree_deserialise_leaves(ckpt['rew_end_model'], self.rew_end_model)
            # self.rew_end_model.load_state_dict(ckpt['rew_end_model'])

        self.state_decoder.eval()
        # self.rew_end_model.eval()
        # # JAX denoiser (no eval mode needed)
        # self.rew_end_model_infer = eqx.nn.inference_mode(self.rew_end_model)
    ...
    def step(self, rollout):
        ...
        for i in pbar:
            ...
            samples = self.replay_buffer.sample_batch(batch_num_samples=64,
                                                      ...
                                                      valid_sample=False)
            time_c[0] += time.time() - time_s
            time_s = time.time()
            samples = samples.to(self.config.DEVICE)
            time_c[1] += time.time() - time_s
            time_s = time.time()
            
            # Normalize and preprocess
            samples.shared_obs = self.normalize_state(samples.shared_obs.mean(2))
            samples.shared_obs[samples.mask_padding.logical_not()] = torch.zeros_like(
                samples.shared_obs[samples.mask_padding.logical_not()], device=self.config.DEVICE
            )
            time_c[2] += time.time() - time_s
            time_s = time.time()

            # Convert to JAX arrays
            shared_obs_jax = jnp.array(samples.shared_obs.cpu().numpy())
            # Encode using JAX denoiser
            shared_obs_jax = self.denoiser.encode(shared_obs_jax)
            # print(samples)
            act_jax = jnp.array(samples.act.cpu().numpy())
            mask_jax = jnp.array(samples.mask_padding.cpu().numpy())
            time_c[3] += time.time() - time_s
            time_s = time.time()

            # JAX training step
            self.jax_key, subkey = jax.random.split(self.jax_key)
            self.denoiser, self.denoiser_opt_state, loss, metrics = train_step(
                self.denoiser, self.denoiser_opt, self.denoiser_opt_state, shared_obs_jax, act_jax, mask_jax, subkey
            )
            time_c[4] += time.time() - time_s
            time_s = time.time()
            
            # Convert metrics to Python floats
            metrics = {k: float(v) for k, v in metrics.items()}
            time_c[5] += time.time() - time_s
            time_s = time.time()
            ...
            metrics["grad_norm_before_clip"] = 0.0  # Grad clipping handled by optax
            ...
            time_c[6] += time.time() - time_s
        print(f"Denoiser time cost: sample {time_c[0]:.3f}s, to device {time_c[1]:.3f}s, normalize {time_c[2]:.3f}s, encode {time_c[3]:.3f}s, forward {time_c[4]:.3f}s, backward {time_c[5]:.3f}s, others {time_c[6]:.3f}s")
        ...
        to_log = []
        for i in pbar:
            samples = self.mamba_replay_buffer.sample_batch(bs=128, 
                                                            sl=self.config.horizon if self.rew_end_model.__class__ in [TransRewEndModel, TransRewEndModelJax] else 20,
                                                            mode='rew_end_model')
            samples = self._to_device(samples)
            samples['shared_obs'] = self.normalize_state(samples['shared_obs'].mean(2))
            ...
            if self.config.rew_end_model_type != 'rnn':
                attn_mask = self.mamba_replay_buffer.generate_attn_mask(samples["done"],
                                                                        self.rew_end_model.config.tokens_per_block).to(self.config.DEVICE)
                samples_jax = {k: jnp.array(v.cpu().numpy()) if isinstance(v, torch.Tensor) else v for k, v in samples.items()}
                attn_mask = jnp.array(attn_mask.cpu().numpy())
                self.rew_end_model, self.rewend_optstate, loss, metrics =train_step_TransRewEndModel(self.rew_end_model, self.rew_end_model_opt, self.rewend_optstate, 
                                          samples=samples_jax, attn_mask=attn_mask, key=0)
            else:
                # Convert samples to JAX arrays
                samples_jax = {k: jnp.array(v.cpu().numpy()) if isinstance(v, torch.Tensor) else v for k, v in samples.items()}
                # print(samples_jax['action'])
                self.rew_end_model, self.rewend_optstate, loss, metrics = train_step_RNNRewEndModel(self.rew_end_model, self.rew_end_model_opt, self.rewend_optstate, 
                                          samples=samples_jax, gamma = self.config.GAMMA, contdisc = self.config.contdisc, key=0)
    ...
    def train_fsq_tokenizer(self, state, obs):
        ...
        self.apply_optimizer(self.state_decoder_optimizer, self.state_decoder, loss, self.config.max_grad_norm)
        self.state_decoder.eval()
        loss_dict = { self.config.vq_type + "/rec_loss": loss.item(), self.config.vq_type + "/active": active_rate, }
    ...
    def train_agent(self, ):
        ...
        obs, shared_obs, act, rew, pcont, end, trunc, logits_act, val, val_bootstrap, av_actions \
            = rollout_diffusion_world_models(
                self.replay_buffer, self.state_rms, self.state_decoder, self.denoiser, self.rew_end_model_infer,
                self.actor, self.critic, self.config, env_type=self.env_type, 
                jax_key=self.jax_key, use_jax=True )
    ...
    def log_compounding_errors(self, trajs):
        ...
        sampler = DiffusionSamplerJax(self.denoiser, self.config.worldmodel_env_cfg.diffusion_sampler)
        ...
        for _ in range(20):
            ...
            rec_obses[segment.filled] = rearrange(
                self.state_decoder.encode_decode(segment.shared_obs[segment.filled].mean(1).to(self.config.DEVICE), True, True),
                'b (n d) -> b n d', n = self.n_agents, d=segment.observation.size(-1), )
            ...
            for i in range(horizon):
                ...
                self.jax_key, subkey = jax.random.split(self.jax_key)
                pred_state_jax, denoised_traj = sampler.sample(
                    jnp.array(state_buffer.cpu().numpy()),
                    jnp.array(act_buffer.cpu().numpy()),
                    subkey
                )
                pred_state = torch.from_numpy(np.array(pred_state_jax)).to(self.config.DEVICE)
