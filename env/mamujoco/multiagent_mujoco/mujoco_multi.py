...
class MujocoMulti(MultiAgentEnv):
    def __init__(self, batch_size=None, **kwargs):
        ...
        self.state_dim = self.share_obs_size

        # COMPATIBILITY
        self.n_obs = self.obs_size
        ...
        self.seed(kwargs['env_args'].get("random_seed"))
        self.check_equal_action_space()
    ...
    def check_equal_action_space(self):
        initial_as = self.true_action_space[0].shape
        for c_as in self.true_action_space[1:]:
            if initial_as != c_as.shape:
                raise NotImplementedError('Currently we donot support different action space in MAMujoco.')
    ...
    def step(self, actions):
        ...
        return (
            self.wrap(self.get_obs()),
            self.wrap(self.get_state()),
            self.wrap(rewards),
            self.wrap(dones),
            self.wrap(infos),
    ...
    def get_state(self, team=None):
        ...
        # Here we wanna keep the same as the original MuJoco environment
        state = self.env._get_obs()
        share_obs = []
        for a in range(self.n_agents):
    ...
    def reset(self, **kwargs):
        ...
        self.steps = 0
        self.timelimit_env.reset()
        return self.wrap(self.get_obs()), self.wrap(self.get_state()), self.get_avail_actions()
